...
export class ClienteService {
  ...
  static async crear(datos: CrearClienteDTO): Promise<ApiResponse<Cliente>> {
    try {
      // Verificar si el RUT ya existe para este usuario
      const clienteExistente = await this.obtenerPorRut(datos.rut, datos.usuario_id)
      if (clienteExistente) {
        return {
          success: false,
      ...
      const clienteCreado = await this.obtenerPorId((result as any).insertId, datos.usuario_id)
